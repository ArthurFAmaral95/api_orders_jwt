...
class NewOrderView(ViewInterface):
  ...
  def handle(self, http_resquest: HttpRequest) -> HttpResponse:
    ...
    self.__validate_inputs(description=description, order_date=order_date, user_id=user_id)
  ...
  def __validate_inputs(self, description: any, order_date: any, user_id: any) -> None:
    if(
      not description
      or not order_date
      or not user_id
